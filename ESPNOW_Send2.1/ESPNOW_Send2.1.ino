 ...
 **************************************
  デバイスはM5StickC、またはM5StackBasic
  17行目、または18行目でどちらかの#includeのコメントを外す。
...
void EspNowSend() {
  ...
  if (esp_now_is_peer_exist(peerInfo.peer_addr) == false) {
    ...
    if ( EspNowErrF(result) > 0) {
      Serial.println(F(": esp_now_add_peer"));
      return;
    }
  }
  ...
  if (peerNum.total_num >= ESP_NOW_MAX_TOTAL_PEER_NUM)
  {
    ...
    if ( EspNowErrF(result) > 0) {
      Serial.println(F(": esp_now_del_peer"));
      return;
    }
  }
  return;
...
void setup() {
  ...
  //LCD表示設定
  //  M5.Lcd.sleep();
  M5.Axp.ScreenBreath(9); //M5StickCの場合(0-12)
  //  M5.Lcd.setBrightness(200); //M5Stackの場合(0-255)
...
void loop() {
  ...
  //    String strData；
  //    strData = "08:3A:F2:68:5C:E4,1,0,100,1,N"; //送信先のマイコンをリセットするプログラム
  //    strData = "08:3A:F2:68:5C:E4,2,20,100,1,RWRGBWRGBWNNNNNNNWNNNBBBBBBBBRNNNNGNNNNBNNNNWRGBWRGBNNNW"; //LEDを光らせるコマンド
  //    strData = "08:3A:F2:68:5C:E4,4";           //受信機側のテストプログラム実行
  //    // データESP-NOWで送信
  //    if (sendF == false) {
  //      Serial.println("busy");
  //    }
  ...
  //    M5.Lcd.setCursor(0, 0);
  //    M5.Lcd.println(F("TanaPika Ready"));
  //      Serial.println("Ready");
  //  }
