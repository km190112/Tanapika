...
#define SOFTWERE_VER 4.0  // 画面に表示するソフトウェアバージョン
...
uint8_t M5_BTN_TEST_CNT = 0;

//String文字列をString配列にカンマごとに分割
uint8_t split(const String data, const char delimiter, String *dst) {
  uint8_t splitCnt = 0;
  uint8_t arraySize = (sizeof(data) / sizeof((data)[0]));
...
//文字列を分割して構造体変数に格納する。
uint8_t dataSplit(const String &str) {
  sendF == false;

  //文字列分割とデータ格納
  String tempStrData[6] = { "\0" };                    // 分割された文字列を格納する配列
  uint8_t splitMaxCnt = split(str, ',', tempStrData);  // 分割数 = 分割処理(文字列, 区切り文字, 配列)
  if (splitMaxCnt != 5) {
    return 1;
  }
...
void setup() {
  ...
  Serial.println(F("TanaPika Send"));
  Serial.print(F("Ver: "));
...
void loop() {
  ...
  if (M5.BtnA.wasPressed()) {
    String testStr = "9C:9C:1F:C1:D5:D0,4,120,0,1,N\n";
    ...
    switch (M5_BTN_TEST_CNT) {
      case 0:
        testStr = "9C:9C:1F:C1:D5:D0,2,120,90,1,BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB\n";
        Serial.println(F("Btn B 0"));
        break;
      case 1:
        testStr = "9C:9C:1F:C1:D5:D0,2,120,90,1,GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG\n";
        Serial.println(F("Btn B 1"));
        break;
      case 2:
        testStr = "9C:9C:1F:C1:D5:D0,2,120,90,1,RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR\n";
        Serial.println(F("Btn B 2"));
        break;
      case 3:
        testStr = "9C:9C:1F:C1:D5:D0,2,120,90,1,WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW\n";
        Serial.println(F("Btn B 3"));
        break;
    }

    M5_BTN_TEST_CNT = M5_BTN_TEST_CNT % 3;
    ...
    String testStr = "9C:9C:1F:C1:D5:D0,3,120,90,1,N\n";
