...
void OnDataSent(const uint8_t *mac_addr, esp_now_send_status_t status) {
  ...
  Serial.println(status == ESP_NOW_SEND_SUCCESS ? ":OK" : ":NG");
...
void setup() {
  ...
  M5.Lcd.println(F("Softwere Ver2.2"));
...
void loop() {
  ...
  while (Serial.available() > 0)       // 受信したデータバッファが1バイト以上存在する場合
  {
    ...
    if (bufp >= DATASIZE)
    {
      ...
      Serial.println(F("ESP_ERR data baffer over"));
      Serial.println("Ready");
    }
    ...
    if (inChar == '\n')
    {
      ...
      if (bufp >= 25) //データのバイト数に不足がない場合に送信
      {
        ...
        Serial.println(F("ESP_ERR data NG"));
      }
      //バッファクリア
      bufp = 0;
      buffArr[bufp] = '\0';
